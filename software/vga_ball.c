...
static void write_cords(vga_ball_cords_t *cords) {
	iowrite32(cords->x, CORDS_X(dev.virtbase) );
	iowrite32(cords->y, CORDS_Y(dev.virtbase) );
	dev.cords = *cords;
}
/*
	...
	switch (cmd) {
		...
		case VGA_BALL_READ_BACKGROUND:
			...
			if (copy_to_user((vga_ball_arg_t *) arg, &vla, sizeof(vga_ball_arg_t)))
				return -EACCES;
			break;
		case VGA_BALL_WRITE_CORDS:
			if (copy_from_user(&vbc, (vga_ball_cords_arg_t *) arg, sizeof(vga_ball_cords_arg_t)))
				return -EACCES;
			write_cords(&vbc.cords);
			break;
		case VGA_BALL_READ_CORDS:
			vbc.cords = dev.cords;
			if (copy_to_user((vga_ball_cords_arg_t *) arg, &vbc, sizeof(vga_ball_cords_arg_t)))
				return -EACCES;
			break;
		default:
			return -EINVAL;
...
{
	vga_ball_color_t teal = { 0x00, 0x80, 0x80 };
	vga_ball_cords_t center = { 319, 239 };
	int ret;
